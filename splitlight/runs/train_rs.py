...
def main(config):
    ...
    # Fix seeds after task init because ClearML may initialize randomness.
    ...
    if config.calc_test_metrics:
        ...
        if config.get("save_test_per_user_metrics", False):
            save_per_user_metrics(recs_test, test_target, config, prefix='test')
...
def create_dataloaders(train, validation, config):
    ...
    if validation_size and (validation_size < len (validation_users)):
        rng = np.random.default_rng(config.dataloader.get("validation_sample_seed", 20260216))
        validation_users = rng.choice(validation_users, size=validation_size, replace=False)
...
def get_checkpoint_name(config):
    ...
    return (
        ...
        f"{model_params}_" +
        f"lr_{config.seqrec_module.lr}_" +
...
def compute_per_user_metrics(recs, test, top_k):
    ranked = recs.copy()
    ranked["rank"] = ranked.groupby("user_id").cumcount() + 1

    targets = test[["user_id", "item_id"]].drop_duplicates()
    users = test[["user_id"]].drop_duplicates().sort_values("user_id").reset_index(drop=True)
    hit_ranks = (
        ranked.merge(targets, on=["user_id", "item_id"], how="inner")
        .groupby("user_id")["rank"]
        .min()
    )

    metrics = users.copy()
    metrics["hit_rank"] = metrics["user_id"].map(hit_ranks)
    for k in top_k:
        hit = metrics["hit_rank"].le(k).fillna(False)
        rank = metrics["hit_rank"].where(hit)
        metrics[f"HR@{k}"] = hit.astype(float)
        metrics[f"MRR@{k}"] = (1.0 / rank).fillna(0.0)
        metrics[f"NDCG@{k}"] = (1.0 / np.log2(rank + 1.0)).fillna(0.0)

    return metrics.drop(columns=["hit_rank"])

def save_per_user_metrics(recs, test, config, prefix='test'):
    top_k = sorted(set(int(k) for k in config.evaluator.top_k if int(k) in {10, 20}))
    if 20 not in top_k:
        top_k.append(20)
    metrics_df = compute_per_user_metrics(recs, test, top_k)
    metrics_df.insert(0, "random_state", config.random_state)
    metrics_df.insert(0, "run_tag", config.get("run_tag", "default"))
    metrics_df.insert(0, "split_name", config.split_name)
    metrics_df.insert(0, "dataset", config.dataset.name)
    ...
    save_dir = os.path.join(
        ...
        config.get("run_tag", "default"),
    )
    os.makedirs(save_dir, exist_ok=True)
    save_path = os.path.join(save_dir, f"{prefix}_per_user_seed_{config.random_state}.csv")
    metrics_df.to_csv(save_path, index=False)
    print(f"saved {prefix} per-user metrics to {save_path}")
...
def save_metrics(metrcis_dict, config, prefix='val'):
    ...
    metrics_df['run_tag'] = config.get("run_tag", "default")
    metrics_df['hidden_units'] = config.model.model_params.get("hidden_units", None)
    metrics_df['num_blocks'] = config.model.model_params.get("num_blocks", None)
    metrics_df['num_heads'] = config.model.model_params.get("num_heads", None)
    metrics_df['dropout_rate'] = config.model.model_params.get("dropout_rate", None)
    metrics_df['lr'] = config.seqrec_module.lr
